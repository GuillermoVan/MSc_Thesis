...
def maximize_epsilon_integer_scaled_fixed_alpha_zero():
    # Create a new Gurobi model
    m = gp.Model("framestack_max_epsilon_integer_scaled_alpha_zero")
    m.Params.OutputFlag = 0  # Disable solver output

    # ── 1. Fixed slack and fixed alpha ────────────────
    eps_slack = 1e-6
    alpha_val = 0.0  # Fixed alpha
    ...
    m.addConstr(epsilon * FS_non_urgent_lower_bound + at_other * beta >= T + eps_slack, name="c1")
    m.addConstr(epsilon * FS_non_urgent_lower_bound + non_linked * beta + delta >= T + eps_slack, name="c2")
    m.addConstr(epsilon * FS_urgent_ub + at_other * beta + gamma + delta <= T, name="c3")
    m.addConstr(epsilon + non_linked * beta + gamma <= T, name="c4")
    
    # ← RELAXED CONSTRAINT c5: changed 0.8 to 1.2
    m.addConstr(epsilon * FS_urgent_ub + at_other * beta + gamma <= epsilon * 1.2, name="c5_relaxed")
    
    m.addConstr(gamma <= epsilon * FS_urgent_ub + at_exit * beta, name="c6")
    m.addConstr(at_exit * beta + gamma <= epsilon * FS_urgent_ub + non_linked * beta, name="c7")
    ...
    if m.status == GRB.OPTIMAL:
        ...
        print("\n--- Raw values ---")
        print(f"α = {alpha_val:.6f} (fixed), β = {beta.X:.6f}, γ = {gamma.X:.6f}, δ = {delta.X:.6f}, ε = {e:.6f}, T = {T.X:.6f}")
...
if __name__ == "__main__":
    maximize_epsilon_integer_scaled_fixed_alpha_zero()
